...
! integer,parameter :: Number_Node=nelx*nely*2
   ...
   real(8),parameter :: Convergence_Error=1.0d-8 !収束しやすいよう1桁増した
    ...
    write(*,*) 't=',t  
     ...
     iw=103
      open(iw,file='Convergence_ratio.dat',position='append')
        write(iw,*) Optimization_step,Convergence_ratio(Optimization_step)
      close(iw)
    ...
    open(iw,file=filename,status='replace')
       ...
       write(iw,*)'0.0 setgray'
        ...
        if(eid(e)==4) then
          ...
          write(iw,*) 'closepath'
          write(iw,*) 1.00-(E_str(e)/2.0d0),'setgray'
          write(iw,*) 'fill'
          write(iw,*) ''
        end if
        end do    
