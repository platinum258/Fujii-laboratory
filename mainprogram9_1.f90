program thermalcloak
   ...
   integer Number_Design_Variable,Number_SElement
...
! integer,parameter :: Number_Node=nelx*nely*2
   ...
   real(8),parameter :: Convergence_Error=1.0d-7 !収束しやすいよう1桁増した
  ...
  !  read(IR,*)epb
   ...
   Number_Design_Variable=Number_SEDesignArea
...
xecord(Number_Element),yecord(Number_Element))
  ...
  !  end do   
   ...
   !set element center(領域設定を変えるごとに更新する必要がある)
   e1=0
   e2=0
   do i=0,nelx-1
     do j=1,nely
  ...
  do while(Convergence_Ratio_dowhile>Convergence_Error)
   ...
   do t=1,lambda!while(change>1.0d-2)
     ...
     do i=1,Number_Design_Variable
      ...
      !  end do 
        r1=SElement_Design_Area(i)*2
        r2=r1-1
        lambda_va(r1)=1.0d0+density_filtered_vec(i)
        lambda_va(r2)=1.0d0+density_filtered_vec(i)
     end do
