...
! real(8),parameter :: width=120.0d0
   integer,parameter :: nelx=60
   integer,parameter :: nely=40
...
! integer,parameter :: Number_Node=nelx*nely*2
   ...
   integer,parameter :: Number_Design_Variable=238
   integer,parameter :: Number_EDesignArea=952
   integer,parameter :: SE1=2420
   !optimization step?
   ...
   integer i,j,k,l,m,n,e,t,ir,iw,ie,iel,ier,iek,ik,e1,e2,n1,n2,n3,i1,i2,l1,l2,j1,j2
  !  integer :: rkr_id(SE1)
   integer mepn,npb,npec,epb,ID_Element_Objective_Function_1,node
   integer generation,cr,situation
  ...
  do while(Convergence_Ratio_dowhile>Convergence_Error)
    cr=Optimization_step+1
    call cmaes(design_variable,Convergence_Ratio(cr),&
               'min', Type_CMA,Optimization_Step,Number_Design_Variable, lambda, 0.1d0, 5.0d0, fitness&
              ,average,Vec_D,mat_B,sigma), flg_min, type_cma) 
   
    if(Optimization_step>=1) then
    Convergence_Ratio_dowhile=convergence_ratio(cr)
    end if

    write(*,*)'convergence ratio=', Convergence_Ratio_dowhile

   !$OPM do parallel private(t) 
   ...
   do t=1,lambda!while(change>1.0d-2)
        ...
        do i=0,475
           j=Number_EDesignArea-i
           density_vec(j)=density_vec(i+1)
        end do
   ...
   !$OMP end parallel
    ...
    open(iw,file=filename,status='replace')
       ...
       write(iw,*)'0.0 setgray'
        do e=1,Number_Element
        if(eid(e)==4) then
          write(iw,*) 10.0d0*position_in_element(e,1,1)+300.0d0,10.0d0*position_in_element(e,1,2)+200.0d0, 'moveto'
          write(iw,*) 10.0d0*position_in_element(e,2,1)+300.0d0,10.0d0*position_in_element(e,2,2)+200.0d0, 'lineto'
         write(iw,*) 10.0d0*position_in_element(e,3,1)+300.0d0,10.0d0*position_in_element(e,3,2)+200.0d0, 'lineto'
          write(iw,*) 'closepath'
          write(iw,*) 1.0d0-E_str(e)*0.2d0,'setgray'
          write(iw,*) 'fill'
          write(iw,*) ''
        end if
        end do    
    ...
    if(situation==1) then
    write(*,*) 'objective_function<=0.0025'
    exit
    end if
