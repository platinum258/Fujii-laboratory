...
! integer,parameter :: Number_Node=nelx*nely*2
   ...
   integer,parameter :: SE1=9720
   !optimization step?
   !integer,parameter :: lambda=350 
   real(8),parameter :: Convergence_Error=1.0d-5 !収束しやすいよう一桁増した
   ...
   integer i,j,k,l,m,n,e,t,ir,iw,ie,iel,ier,iek,e1,e2,n1,n2,n3,i1,i2,l1,l2,j1,j2
   integer :: rkr_id(SE1)
  ...
  !  real(8) :: ipmkl(Number_Node)
   ...
   real(8) :: E_str(Number_EDesignArea)
  ...
  do while(Convergence_Ratio_dowhile>Convergence_Error)
    call cmaes(design_variable,Convergence_Ratio(Optimization_step),&
               'min', Type_CMA,Optimization_Step,Number_Design_Variable, lambda, 0.0d0, 5.0d0, fitness)!, flg_min, type_cma) 
   ...
   do t=1,lambda!while(change>1.0d-2)
     ...
     do i=1,Number_EDesignArea
        ...
        lambda_va(iek)=1.0d0+((density_vec(i))/t_flat)
     end do
   ...
   do i=1,SE1
      do j=1,SE1
        rkr(i,j)=rk(rkr_id(i),rkr_id(j))
     end do
   end do     

call mkl_set_num_threads(num_threads)
call dgesv(SE1,1,rkr,SE1,ipmkl,rhsT,SE1,info)
   ...
   !compute objective function
    ...
    open(iw,file=filename,status='replace')
       ...
       write(iw,*)'0.0 setgray'
        do e=1,Number_Element
        if(eid(e)==4) then
          write(iw,*) position_in_element(e,1,1)/2.0d0+30.0d0,position_in_element(e,1,2)/2.0d0+20.0d0, 'm'
          write(iw,*) position_in_element(e,2,1)/2.0d0+30.0d0,position_in_element(e,2,2)/2.0d0+30.0d0, 'l'
          write(iw,*) position_in_element(e,3,1)/2.0d0+30.0d0,position_in_element(e,3,2)/2.0d0+30.0d0, 'l'
          write(iw,*) 'closepath'
