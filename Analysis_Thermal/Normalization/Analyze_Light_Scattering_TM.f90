...
subroutine Analyze_Light_Scattering_TM &
       ( Flag_Incident_Wave, Flag_PEC_Boundary_Condition, Flag_Material, &
         ...
         !============================================================================================
         Field_Electric_All, Field_Electric_Incident, Field_Electric_Scattering )
   ...
   complex( kind( 0d0 ) ), intent(out) :: Field_Electric_Scattering( Number_Node )
   complex( kind( 0d0 ) ), intent(out) :: Field_Electric_All( Number_Node )
 
   integer :: i, j, k, e, Counter 
   ...
   !PEC
   integer, allocatable, dimension(:) :: Flag_Element_Triangle_PEC_BC
   ...
   integer, allocatable, dimension(:) :: NodeNumber_Dirichlet_BC, Flag_Node_Dirichlet_BC, Flag_Node_PEC_BC_Fixed
   ...
   integer :: ID_Element_PEC, Flag_Remove_Element
   ...
   !============================================================================================
   write(*,*)'   call Analyze_Light_Scattering_TM '
   !============================================================================================
   ...
   =abs( Difference_Position_Node_Element_Triangle( 1, 1, 3, : ) & 
      ...
      *Difference_Position_Node_Element_Triangle( 2, 1, 3, : ) )/2d0
   ...
   do e= 1, Number_Element_Triangle
      if( Area_Element_Triangle( e )==0.0d0 )then
         ...
         call Output_Error( 'Analyze_Light_Scattering_TM', 166 )
      end if
   end do 
   ...
   BasisFunction_B( 1, : ) &
   = Difference_Position_Node_Element_Triangle( 2, 2, 3, : )/( 2d0 *Area_Element_Triangle( : ) )

   BasisFunction_B( 2, : ) & 
   = Difference_Position_Node_Element_Triangle( 2, 3, 1, : )/( 2d0 *Area_Element_Triangle( : ) )

   BasisFunction_B( 3, : ) & 
   = Difference_Position_Node_Element_Triangle( 2, 1, 2, : )/( 2d0 *Area_Element_Triangle( : ) )

   BasisFunction_C( 1, : ) & 
   = Difference_Position_Node_Element_Triangle( 1, 3, 2, : )/( 2d0 *Area_Element_Triangle( : ) )

   BasisFunction_C( 2, : ) & 
   = Difference_Position_Node_Element_Triangle( 1, 1, 3, : )/( 2d0 *Area_Element_Triangle( : ) )

   BasisFunction_C( 3, : ) & 
   = Difference_Position_Node_Element_Triangle( 1, 2, 1, : )/( 2d0 *Area_Element_Triangle( : ) )
   ...
   !===============================
      do i= 1, Number_Element_Triangle
         if( Class_Element_Triangle( i ) == ID_Element_Material )then
            ...
            Dielectric_Constant_Triangle( i )= Dielectric_Constant_OpenRegion
            ...
            write(*,*)'  Analyze_Light_Scattering_TM.f90 152'
         end if
      end do 
   !===============================
   ...
   !===============================
      do i= 1, Number_Element_Triangle
         if( Class_Element_Triangle( i ) == ID_Element_Material )then
            ...
            write(*,*)'ERROR'
            write(*,*)'  Class_Element_Triangle( i )=', Class_Element_Triangle( i )
            write(*,*)'  Analyze_Light_Scattering_TM.f90 174'
         end if
      end do
   !===============================
   ...
   !===============================
      call Output_Error( 'Analyze_Light_Scattering_TM', 598 )
   end if
   ...
   !$omp shared( Class_Element_Square, ID_Element_PML_Xp, ID_Element_PML_Xm )&
   !$omp shared( ID_Element_PML_Yp, ID_Element_PML_Ym, ID_Element_PML_XpYp )&
   !$omp shared( ID_Element_PML_XpYm, ID_Element_PML_XmYp, ID_Element_PML_XmYm ) 
   do e= 1, Number_Element_Square
      do j= 1, Number_Point_Gauss_Legendre_Element_Square 
   ...
   LocalMatrix_Triangle_Scattering_tmp= Zero 
   LocalMatrix_Triangle_Scattering= Zero 
   LocalMatrix_Triangle_Scattering_RHS= Zero 
   ...
   !==============================================================================================================
      ...
      LocalMatrix_Triangle_Scattering_tmp( :, :, : ) & 
      = 1d0/2d0*Coefficient_C( :, :, : ) +1d0/6d0*Coefficient_Xi( :, :, : ) &
       +1d0/6d0*Coefficient_Eta( :, :, : ) +1d0/12d0*Coefficient_Xi_Xi( :, :, : ) &
       +1d0/12d0*Coefficient_Eta_Eta( :, :, : ) +1d0/24d0*Coefficient_Xi_Eta( :, :, : )
      ...
      !$omp shared( Area_Element_Triangle, Dielectric_Constant_Triangle, LocalMatrix_Triangle_Scattering_tmp ) &
      !$omp shared( BasisFunction_B, BasisFunction_C, Frequency_Normalized ) & 
      !$omp shared( Class_Element_Triangle, Dielectric_Constant_OpenRegion )
      do e= 1, Number_Element_Triangle
         !if( Class_Element_Triangle( e ) /= ID_Element_PEC )then
            do i= 1, 3
               do j= 1, 3
      ...
      !==============================================================================================================
      write(*,*)'      =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-='
      write(*,*)'      Implement Boundary Integral'
      write(*,*)'      =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-='
      !==============================================================================================================
   ...
   !==============================================================================================================
      call Output_Error( 'Analyze_Light_Scattering_TM', 838 )
   end if
   ...
   do e= 1, Number_Element_Square
      if( Class_Element_PML_Material( e )==ID_Element_PML_Air )then
         ...
         call Output_Error( 'Analyze_Light_Scattering_TM', 751 )
      end if
   end do
   ...
   !!$omp shared( GlobalMatrix_RHS, J_GlobalMatrix_RHS, Field_Electric_Incident )
   do i= 1, Number_Node
      do j= 1, Width_Matrix_LHS
         if( J_GlobalMatrix_RHS( i, j )/=0 )then
            Global_Vector_RHS( i )&
            = Global_Vector_RHS( i ) &
             +GlobalMatrix_RHS( i, j ) *Field_Electric_Incident( J_GlobalMatrix_RHS( i, j ) )
         end if
      end do
   end do
   ...
   !==================================================================================================
      ...
      Nodal_Value_on_Dirichlet_BC( : )= -Field_Electric_Incident( : )
      Flag_Node_Dirichlet_BC= Flag_Off

      !if( Position_Center_FixedDomain_Y_Cloaked_Region==-Size_Y_Scattering )then
      if( Flag_Thermal_Device==51 )then
         do e= 1, Number_Element_Square
            if( Class_Element_Square( e ) == ID_Element_PML_Ym .or. & 
                Class_Element_Square( e ) == ID_Element_PML_XpYm .or. &
                Class_Element_Square( e ) == ID_Element_PML_XmYm )then
               do i= 1, 4
                  Flag_Node_Dirichlet_BC( Index_Element_2_Node_Square( i, e ) )= Flag_On
               end do
            end if
         end do
      end if 
   ...
   !==================================================================================================
   end if ! Flag_PEC_Boundary_Condition==1
   !==================================================================================================
   ...
   do j= 1, Width_Matrix_LHS
      do i= 1, Number_Node
         if( Flag_Symmetric==Flag_On )then
            ...
            call Output_Error( 'Analyze_Light_Scattering_TM', 1118 )
         end if
      end do
   end do
   ...
   call ME57( Number_Node, Number_NonZero, aa_zmumps, ia_zmumps, ja_zmumps, Global_Vector_RHS, Field_Electric_Scattering )

   deallocate( aa_zmumps )
   deallocate( ia_zmumps )
   deallocate( ja_zmumps )

   Field_Electric_All( : )= Field_Electric_Scattering( : ) +Field_Electric_Incident( : )
   ...
   return
end subroutine Analyze_Light_Scattering_TM
